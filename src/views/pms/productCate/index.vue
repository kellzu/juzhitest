<template>
  <div class="app-container">
    <el-card class="operate-container" shadow="never">
      ...
      <span style="margin-top: 5px">父级分类：</span>
      <a v-for="(item,index) in parentName" :key="index">{{item}}/</a>
...
export default {
  ...
  watch: {
    $route(route) {
      this.resetParentId();
      if (route.query.parentName == null) {
        this.parentName.pop();
      }
      this.getList();
    }
  ...
  methods: {
    resetParentId() {
      ...
      if (this.$route.query.parentName != null) {
        var parentTypeList = this.$route.query.parentName;
        this.parentName = this.$route.query.parentName;
      }
    },
    handleAddProductCate() {
      this.$router.push({path:"/pms/addProductCate",query:{parentId:this.parentId}});
    },
    getList() {
    ...
    handleShowNextLevel(index, row) {
      this.parentName.push(row.name);
      this.$router.push({
        path: "/pms/productCate",
        query: { parentId: row.id, parentName: this.parentName }
      });
    },
